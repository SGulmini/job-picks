    ...
    const generatePrompt = (template: string, mode: "long" | "short" | "very_short" | "creative", customInstructions?: string, isCustomTemplate: boolean = false) => {
      ...
      if (mode === "creative") {
        return [
          ...
          "- Do NOT use standard cover letter structure (no formal headers, no 'I am writing to apply').",
          "- If you include any greeting/salutation, ALWAYS use 'Dear Recruiting Team' (or the equivalent in the target language). NEVER use a specific company name or person's name in any greeting.",
      ...
      return [
        ...
        "- SALUTATION RULE: ALWAYS use 'Dear Recruiting Team' (or the equivalent in the target language, e.g. 'Chère équipe de recrutement' in French, 'Sehr geehrtes Recruiting-Team' in German, 'Gentile team di selezione' in Italian). NEVER use a specific company name or person's name in the salutation. Even if the template contains a company name in the greeting, replace it with the generic 'Dear Recruiting Team' equivalent.",
        "",
